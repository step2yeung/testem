...
module.exports = class BrowserTestRunner {
  constructor(launcher, reporter, index, singleRun, config) {
    ...
    this.browserUrl = launcher.settings.test_page;
  ...
  exit() {
    ...
    log.info(`Closing browser ${this.name()} for url ${this.browserUrl}.`);
  ...
  tryAttach(browser, id, socket) {
    ...
    socket.on('browser-console', function(/* ...args */) {
      ...
      if (message.indexOf('emitting browser-login') > 0) {
        console.log(message);
      }
      this.logs.push(
  ...
  onAllTestResults() {
    log.info(`Browser ${this.name()} for url ${this.browserUrl} finished all tests.`, this.singleRun);
    this.onEnd();
  }
  ...
  onGlobalError(msg, url, line) {
    ...
    log.info(`Global Error event received from for id:${this.launcherId} url:${this.browserUrl}.`);
    log.info(message);
  ...
  onDisconnect() {
    log.info(`Disconnect event received from for id:${this.launcherId} url:${this.browserUrl}.`);
    this.socket = null;
  ...
  onProcessExit(code) {
    log.info(`Process Exit event received from for id:${this.launcherId} url:${this.browserUrl}.`);
  ...
  onProcessError(err) {
    log.info(`Process Error event received from for id:${this.launcherId} url:${this.browserUrl}.`);
    let browserProcess = this.process;
    this.process = null;
